...
const ScheduleNewClass = () => {
  ...
  const courseHandleChange = async (val) => {
    console.log(val);
    const res1 = await axios.post('http://18.140.159.50:3333/api/admin-subjects',{"course_id": val})
    setSubjectList(res1.data);
    const res2 = await axios.post('http://18.140.159.50:3333/api/get-batches-course-id',{"course_id": val})
    setBatcheslist(res2.data.data);
  }
  const getSelectOptionsData = async () => {
    const res1 = await axios.post('http://18.140.159.50:3333/api/admin-category');
    setCourseCatelist(res1.data.data);
    const res2 = await axios.get('http://18.140.159.50:3333/api/get-all-courses');
    setCourseslist(res2.data);
    setCourseslistdummy(res2.data);
    const res3 = await axios.post('http://18.140.159.50:3333/api/get-batches-course-id',{"course_id": 22})
  ...
  const getClassById = async (id) => {
    const res1 = await axios.get(`http://18.140.159.50:3333/api/students/course/classes/${id}`)
  ...
  return (
    ...
    >
      {
        courseCatelist.length!==0 && courseCatelist.map((elem,i)=>{
    ...
    >
      {
        courseslist.length!==0 && courseslist.map((elem,i)=>{
          return  <Option key={i} value={elem.id}>{elem.course_name}</Option>
        })
      }
    </Select>
        </Form.Item>
    ...
    >
      {
        subjectList.length!==0 && subjectList.map((elem,i)=>{
          return  <Option key={i} value={elem.id}>{elem.subject_name}</Option>
        })
      }
    </Select>
    ...
    >
      {
        batcheslist.length!==0 && batcheslist.map((elem,i)=>{
          return  <Option key={i} value={elem.id}>{elem.batch_id}</Option>
        })
      }
    </Select>
        </Form.Item>
    ...
    >
      {
        venueslist.length!==0 && venueslist.map((elem,i)=>{
          return  <Option key={i} value={elem.id}>{elem.venue_name}</Option>
        })
      }
    </Select>
        </Form.Item>
