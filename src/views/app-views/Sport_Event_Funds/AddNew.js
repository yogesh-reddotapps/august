...
export default function AddNew() {
    ...
    useEffect(() => {
      if (location.pathname===`/app/events/sport-event-funds/update/${param.id}`) {
        getMembershipPlan();
      }
    }, [])
            ...
            >
                <div className='border rounded p-3'>
                    <div style={{ gap: "60px" }} className='d-flex '>
                        <div style={{ width: "45%", }}>
                            <Form.Item name='id' label="Sports Function" rules={[{ required: true, message: 'Please enter Sports Function' }]} >
                                <Input placeholder="Sports Function" />
                            </Form.Item>
                            <Form.Item name='date' label="Date of Function" >
                                <DatePicker placeholder='DD/MM/YYYY' style={{ width: "100%" }} format='DD/MM/YYYY' />
                            </Form.Item>
                        </div>
                        <div style={{ width: "45%", }}>
                            <Form.Item name='name' label="Requested By" rules={[{ required: true, message: 'Please enter Requested By' }]} >
                                <Input placeholder="Full Name" />
                            </Form.Item>
                            <Form.Item name='amount' label="Fund Amount">
                                <InputNumber min={0} style={{ width: "100%" }} prefix={<CustomIcon svg={dollars} className="site-form-item-icon" />} placeholder="Amount" />
                            </Form.Item>
                        </div>
                    </div>
                    <Form.Item name='perks' label="Fund Request Details">
